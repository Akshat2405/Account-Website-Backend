...
router.post('/transaction',authenticate,async (req,res)=>{
    try{
        const{accountNo,email,senderAccountNo,amount}=req.body;
        // console.log(req.body);
        if(!accountNo || !email || !senderAccountNo || ! amount){
            console.log("error in response form");
            return res.status(422).json({error:"Plz filling the field properly"});
        }
        // console.log("error in response form");
        const senderContact=await User.findOne({_id:senderAccountNo});
        let deposited=true;
        if(senderContact){
            // console.log("error in response form");
            const userTransaction=await senderContact.addTransaction(email,senderAccountNo,amount,deposited);
            await senderContact.save();
        }
        const userContact=await User.findOne({_id:accountNo});
        deposited=false;
        if(userContact){
            const userTra=await userContact.addTransaction(email,accountNo,amount,deposited);
            await userContact.save();
        }
        if(senderContact && userContact){
            return res.status(201).json({message:"user save message succesfully"});
        }
    }
    catch(err){
        return res.status(400).json({error:"INVALID sender account No"});
    }
})
